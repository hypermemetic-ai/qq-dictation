...
| [Dictation pipeline lifecycle](runtime/dictation-pipeline.md) | [`TranscriptionCoordinator`, `Stage`, and `OperationOwner`](runtime/dictation-pipeline.md#responsibility-and-entrypoints), [input timing and local controls](runtime/dictation-pipeline.md#input-timing-and-local-controls), [capture/VAD](runtime/dictation-pipeline.md#capture-vad-and-microphone-lifetime), [stop-to-delivery ordering](runtime/dictation-pipeline.md#stop-to-delivery-ordering), streaming fallback, cancellation, and overlay behavior. |
...
- `TranscriptionCoordinator` is the sole local/remote operation authority; every non-idle transition and effect is matched to the exact `OperationOwner`. Explicit CLI pane targets are validated only on idle start, retained through stop, and never used to interfere with remote ownership.
...
| q-mode CLI controls, remote protocol/PTT, Herdr targeting, clipboard, typing, or injection | [Remote and delivery](runtime/remote-and-delivery.md) | `cli::{CliArgs,running_instance_command}`; `StartTarget`; coordinator `start_or_stop`/`remote_*`; `target_binding`; `clipboard::paste`; `input`; `ops/install/handy-remote-*.py` | `cli::tests`; `transcription_coordinator::tests`; `remote::tests`; `target_binding::tests`; `clipboard::tests`; `tests.test_{remote_helpers,remote_laptop_client}` | Rust boundary + matching Python suite; live SSH/X11/Herdr only when behavior changes |
...
| Build, package, dependency, installer, service, rollback, CI, or Check | [Build/install/Check](operations/build-install-check.md) | `ops/build/*`; `src-tauri/{Cargo.toml,build.rs,tauri.conf.json}`; `ops/install/{install-local.sh,handy.service}`; `tools/check.sh`; `.github/workflows/*` | `tests.test_install_local_service`; root Python suite; affected Cargo tests | Installer suite first; contained build for packaging; `tools/check.sh` only on committed target |
...
Source plus tests outrank prose when they disagree. In particular, workstation q mode is externally owned while the remote laptop PTT key is `Control_L`; registered commands/events—not a remembered count—define IPC; `src/bindings.ts` is generated; catalog capabilities are advisory until load; successful autostart/settings optimism does not prove the native effect; a build marker or Check receipt records evidence but is not a trust anchor. Ordinary CI is split and has path omissions, so select validation from the table rather than assuming one workflow covers the change.
...
- **Remote target delivery:** prove changed SSH, PipeWire, laptop X11 grabs/focus, systemd user services, Ghostty/Herdr identity, and target rendering end to end. Anchors: `remote.rs`, `target_binding.rs`, `ops/install/handy-remote-*.py`. Reason: repository tests use protocol/state-machine doubles and cannot establish a live external session or exactly-once recipient outcome.
