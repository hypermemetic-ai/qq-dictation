...
3. Send `finish`. It returns `processing` while the workstation finalizes the stream and records history under the existing WAV policy.
...
After receiving and strictly validating that plan, the laptop again requires a readable current X11 focus immediately before the effect. It does not compare this focus with either the pre-commit focus or any start-time window. It marks the injection attempted, invokes the configured absolute `xdotool` once as `type --delay 0 --clearmodifiers -- TEXT`, and, only if that text attempt reports success and `submit_key` is not null, invokes at most one corresponding `key --clearmodifiers ...` attempt. There is no retry, partial re-injection, saved-target activation, Herdr delivery, or other fallback after an absent focus, malformed handoff, timeout, adapter error, or uncertain effect.
