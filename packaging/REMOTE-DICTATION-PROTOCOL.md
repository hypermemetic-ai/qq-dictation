...
5. `finish` changes status to `processing`; poll `status` until `succeeded`, `failed`, or `cancelled`. `cancel` is valid only for the owning connection and request. Pending/recording cancellation replies `cancelled` after the workstation is idle. Processing cancellation replies and polls as `cancelling` until the processing completion guard records terminal `cancelled` and returns the workstation pipeline to idle.

The pending target claim lives for 5 seconds. The entire request lives for 10 minutes. One helper connection may perform a sequence of non-overlapping requests while mode remains armed. A terminal status or immediate terminal cancellation retires its current request; old controls remain stale and the next `start` receives a new app-minted ID. A 30-second read timeout disconnect-cancels an active request, but an idle timeout between requests is normal armed-mode idleness and does not close the helper. Timeout after any part of a frame remains a truncated protocol failure.

A helper disconnect cancels its recording or processing request. Request IDs and binder claims are one-shot; stale, cross-connection, out-of-order, malformed, truncated, oversized, non-live, and replayed input is refused rather than normalized. The bound pane is immutable, and remote handling never consults workstation X11 capture or Herdr session-global focus.
