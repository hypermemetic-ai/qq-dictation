...
The workstation helper `handy-remote-stream.py` is invoked through the existing SSH account. It copies stdin/stdout bytes to `$XDG_RUNTIME_DIR/qq-dictation/remote.sock`; it has no transcription or target policy. The app creates that same-user Unix socket as mode `0600` beneath an owner-only directory, verifies Linux peer credentials, and has no TCP listener or protocol credential.
...
2. The laptop invokes the configurable Herdr transport chord in band. The installer-owned candidate is `prefix+alt+d`, subject to the later laptop/Ghostty live proof. Herdr runs `handy-remote-bind.py` detached with `HERDR_ACTIVE_PANE_ID`. The binder opens its own socket connection and sends `{"type":"bind","version":1,"pane_id":"..."}`. It succeeds only for the sole live pending claim and an exact live pane.
